...
class MarkFeaturesPopup extends SdkPopup {
	...
	mover(evt) {
		this.props.canMovePopup()
	}
	markFeatures(evt) {
		const feature_ids = [];
		const features = this.props.features;

		for (let i = 0, ii = features.length; i < ii; i++) { // create an array of ids to be removed from the map.
		...
		return this.renderPopup((
			<div className='sdk-popup-content custom-popup'>
				<button ref={
					(c) => {
						if (c) {
							c.addEventListener('click', this.mover);
						}
					}
				}>Move Popup</button>
				<br />
...
class App extends React.Component {
	...
	componentDidMount() { // Start with a reasonable global view of the map.
		...
		store.dispatch(mapActions.addLayer({ id: 'osm', source: 'osm', type: 'raster' }));
 ...
 */
	addPoints(sourceName, n_points = 10) {
		for (let i = 0; i < n_points; i++) { // the feature is a normal GeoJSON feature definition
		...
		return (
			<div className="App">
				<Provider store={store}>
					<SdkMap className={this.state.movingPopup ? 'crosshair map' : 'map'} includeFeaturesOnClick
						onClick={
							(map, xy, featuresPromise) => {
								featuresPromise.then((featureGroups) => { // setup an array for all the features returned in the promise.
									let features = [];

									// featureGroups is an array of objects. The key of each object
									// is a layer from the map.
									for (let g = 0, gg = featureGroups.length; g < gg; g++) { // collect every feature from each layer.
									...
									if (this.state.movingPopup) {
										console.log('Popup is in move', this.state.popupFeatures)
										this.setState({
											movingPopup: false,
										})
										map.addPopup(
											<MarkFeaturesPopup coordinate={xy}
												features={this.state.popupFeatures}
												closeable
												canMovePopup={() => { this.popupMover() }}>
												<i>No features found.</i>
											</MarkFeaturesPopup>
										);
									} else {
										this.setState({ popupFeatures: features });
										if (features.length === 0) { // no features, :( Let the user know nothing was there.
											map.addPopup(
												<MarkFeaturesPopup coordinate={xy}
													features={features}
													closeable
